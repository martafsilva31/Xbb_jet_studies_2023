...
#include "fastjet/contribs/AltVariableR/AltVariableRPlugin.hh"
...
void FastJetFinder::Init()
{
  ...
  fMinR = GetDouble("MinR", 0.02);
  fMaxR = GetDouble("MaxR", 0.4);
  //fClusterType = fastjet::contrib::ClusterType::AKTLIKE;//GetDouble("fClusterType", -1); // AKTLIKE

  //-- Alt Variable-R parameters --
  fAltRho = GetDouble("AltRho", 30);
  fAltMinR = GetDouble("AltMinR", 0.02);
  fAltMaxR = GetDouble("AltMaxR", 0.4);
  ...
  switch(fJetAlgorithm)
  {
    ...
    case 8:
      ...
      break;
    case 9:
      plugin = new VariableRPlugin(fRho, fMinR, fMaxR, -1);
      fDefinition = new JetDefinition(plugin);
      break;
    case 10:
      plugin = new AltVariableRPlugin(fAltRho, fAltMinR, fAltMaxR, -1);
      fDefinition = new JetDefinition(plugin);
      break;
    default:
      std::cout << " ====> One of the jet algorithms chosen is not implemented!!!" << std::endl;
      std::cout << " ====> ....running with the default Anti-kT!!!" << std::endl;
      fDefinition = new JetDefinition(antikt_algorithm, fParameterR);
      break;
  }
