//  AltVariableR Package
//  Questions/Comments?  jthaler@jthaler.net
//
//  Copyright (c) 2009-2016
//  David Krohn, Gregory Soyez, Jesse Thaler, and Lian-Tao Wang
//
//  $Id: AltVariableR.cc 596 2014-04-16 22:15:15Z jthaler $
//----------------------------------------------------------------------
...
namespace contrib {
  ...
  const double AltVariableRPlugin::CALIKE  =  0.0;
  const double AltVariableRPlugin::KTLIKE  =  1.0;
  const double AltVariableRPlugin::AKTLIKE = -1.0;
   
  //----------------------------------------------------------------------
  // classes to help run a Variable R algorithm using NN-type classes

  // class carrying particle-independent information
  class AltVariableRNNInfo {
  public:
    AltVariableRNNInfo(double rho2_in, double min_r2_in, double max_r2_in,
                    AltVariableRPlugin::ClusterType clust_type_in)
  ...
  private:
    ...
    AltVariableRPlugin::ClusterType _clust_type;
  };

  // class carrying the minimal info required for the clustering
  class AltVariableRBriefJet {
  public:
    void init(const PseudoJet & jet, AltVariableRNNInfo *info) {
      _rap = jet.rap();
      _phi = jet.phi();
      ...
      // AltVR DAN
      //_beam_R2 = pow(pow((pow(pow(1/sqrt(info->max_r2()),1/0.1) + pow(sqrt(pt2)/sqrt(info->rho2()),1/0.1),-1)+pow(sqrt(info->min_r2()),1/0.1)),0.1),2);
      ...
      _beam_R2 = pow(0.0173 + exp(-0.854-0.00001148*sqrt(pt2)*pow(10,3)),2);
      if      (_beam_R2 > info->max_r2()){ _beam_R2 = info->max_r2();}
      else if (_beam_R2 < info->min_r2()){ _beam_R2 = info->min_r2();}
    ...
    double geometrical_distance(const AltVariableRBriefJet * jet) const {
    ...
    double distance(const AltVariableRBriefJet * other_bj_jet){
  ...
  //----------------------------------------------------------------------
  // now the implementation of AltVariableR itself
  //----------------------------------------------------------------------
  LimitedWarning AltVariableRPlugin::_preclustering_deprecated_warning;
  ...
  AltVariableRPlugin::AltVariableRPlugin(double rho, double min_r, double max_r,
    ...
    if (min_r < 0.0)              throw Error("AltVariableRPlugin: Minimum radius must be positive.");
    if (precluster && min_r==0.0) throw Error("AltVariableRPlugin: To apply preclustering, minimum radius must be non-zero.");
    if (max_r < 0.0)              throw Error("AltVariableRPlugin: Maximum radius must be positive.");
    if (min_r > max_r)            throw Error("AltVariableRPlugin: Minimum radius must be bigger than or equal to maximum radius.");
    ...
    if ((requested_strategy!=Best) && (requested_strategy!=Native) && (requested_strategy!=NNH))
      throw Error("AltVariableRPlugin: with FastJet<3.2.0, Native, Best, and NNH are the only supported strategies");
#endif
    ...
    if (precluster){
      ...
      if ((requested_strategy!=Best) && (requested_strategy!=Native))
        throw Error("AltVariableRPlugin: pre-clustering is only supported for the Native and Best strategies");

      _preclustering_deprecated_warning.warn("AltVariableRPlugin: internal pre-clustering is deprecated; use the NestedDefs FastJet plugin instead.");
    }  
  }

  //----------------------------------------------------------------------
  // Description of algorithm, including parameters
  string AltVariableRPlugin::description() const{
  ...
  //----------------------------------------------------------------------
  // do the clustering
  void AltVariableRPlugin::run_clustering(ClusterSequence & cs) const {
    ...
    AltVariableRNNInfo info(_rho2, _min_r2, _max_r2, _clust_type);

#if FASTJET_VERSION_NUMBER >= 30200
    if (strategy==N2Tiled){
      NNFJN2Tiled<AltVariableRBriefJet,AltVariableRNNInfo> nnt(cs.jets(), _max_r, &info);
      _NN_clustering(cs, nnt);
    } else if (strategy==N2Plain){
      NNFJN2Plain<AltVariableRBriefJet,AltVariableRNNInfo> nnp(cs.jets(), &info);
      _NN_clustering(cs, nnp);
    } else { // NNH is the only option left
#endif
      fastjet::NNH<AltVariableRBriefJet,AltVariableRNNInfo> nnh(cs.jets(), &info);
  ...
  AltVariableRPlugin::Strategy AltVariableRPlugin::_best_strategy(unsigned int N) const{
#if FASTJET_VERSION_NUMBER >= 30200
  ...
  void AltVariableRPlugin::_NN_clustering(ClusterSequence &cs, NN &nn) const{
  ...
  void AltVariableRPlugin::_preclustering(ClusterSequence & cs, set<int>& unmerged_jets) const {
    int cntr(0);
  ...
  void AltVariableRPlugin::_native_clustering(ClusterSequence & cs) const {
  ...
  void AltVariableRPlugin::_merge_jet_with_beam(ClusterSequence & clust_seq, JetDistancePair & jdp, set<int>& unmerged_jets) const{
  ...
  void AltVariableRPlugin::_merge_jets(ClusterSequence & clust_seq,
                                    JetDistancePair & jdp,
  ...
  void AltVariableRPlugin::_setup_distance_measures(ClusterSequence & clust_seq,
                                                 vector<JetDistancePair> &jet_vec,
                                                 set<int> & unmerged_jets) const{
    JetDistancePair jpair;
  ...
  double AltVariableRPlugin::_get_JJ_distance_measure(const PseudoJet& j1, const PseudoJet& j2) const {
    ...
    std::cout << "inesochoa: bla!"<< std::endl;
  ...
  // Different measures for AKTLIKE, CALIKE, and KTLIKE, as well as for GenKT
  double AltVariableRPlugin::_get_JB_distance_measure(const PseudoJet& jet) const{
    double pre_factor = pow(jet.perp2(), _clust_type);
    double geom_factor = _rho2 / jet.perp2();
    double p_T = sqrt(jet.perp2());
    ...
    double R_sc_alt = pow(0.0173 + exp(-0.854-0.00001148*sqrt(jet.perp2())*pow(10,3)),2);
    return R_sc_alt * pre_factor;
    
    // AltVR Dan

    // double R_smooth_2 = pow(pow((pow(pow(1/sqrt(_max_r2),1/0.1) + pow(sqrt(p_T)/sqrt(_rho2),1/0.1),-1)+pow(sqrt(_min_r2),1/0.1)),0.1),2);
    // return R_smooth_2 * pre_factor;
  }
